...
def create_latex_from_images(image_folder, rapport_tex, rapport):
    ...
    # Ajouter les images au document LaTeX
    for image_name in os.listdir(image_folder):
        if image_name.lower().endswith(('.png', '.jpg', '.jpeg', '.pdf')):
            image_path = os.path.join(image_folder, image_name)
            if os.path.isfile(image_path):
                latex_content += r'\begin{figure}[h!]\centering\includegraphics[width=\textwidth]{' + image_path.replace('\\', '/') + r'}\caption{' + image_name + r'}\end{figure}'
